...
# https://github.com/abevoelker/simple_form_tailwind_css/issues/8
class Bklt::FormBuilder < SimpleForm::Tailwind::FormBuilder
  def error_notification(options = {})
    # Your logic here to set options from your config or whatever:
    options = {
      icon_classes: "h-5 w-5 text-red-400 dark:text-red-600",
      message_classes: "text-sm font-semibold text-red-500 dark:text-red-500",
      border_classes: "bg-red-100 bg-opacity-10 rounded-md border-red-200 p-2 dark:border-red-800 border",
      icon: "exclamation-circle"
    }.merge(options)

    SimpleForm::Tailwind::ErrorNotification.new(self, options).render # or `super`
  end
end
...
SimpleForm.setup do |config|
  ...
  config.wrappers :default, tag: "div", class: "", error_class: "", valid_class: "" do |b|
    ...
    b.wrapper tag: "div", class: "flex justify-between" do |d|
      d.use :label, class: "form-label"
      d.use :hint, wrap_with: { tag: :p, class: "mt-1 text-sm text-gray-500" }
    end
    b.use :input,
      class: "form-input",
      error_class: "form-input form-input-error "
    b.use :full_error, wrap_with: { tag: "p", class: "mt-1 text-xs font-semibold text-red-400 dark:text-red-600" }
  end
  ...
  config.wrappers :append_string, tag: "div", class: "", error_class: "", valid_class: "" do |b|
    ...
    b.wrapper tag: "div", class: "flex rounded-md shadow-sm border border-gray-200 dark:border-gray-800" do |d|
      d.use :input,
        class: "flex-1 min-w-0 form-input !border-0",
      ...
      d.wrapper tag: :span, class: "inline-flex items-center px-3 rounded-r-md border border-l-0 border-gray-300 bg-gray-50 text-gray-500 sm:text-sm" do |e|
        d.use :append
      end
    end
    b.use :full_error, wrap_with: { tag: "p", class: "mt-2 text-sm text-red-600" }
  ...
  config.wrappers :error_notification, icon_classes:  "h-5 w-5 text-accent",
    message_classes: "text-sm text-red-700 dark:text-red-300",
    border_classes: "bg-red-050 border-l-4 border-red-400 p-4 dark:bg-red-950 dark:border-red-600",
    icon: "x-circle"
